...
V = VideoWriter('videoTracking.avi');
open(V);
...
for i=1:n 
    ...
    if i==1
        imshow(image);
        [x,y] = ginput(6); %4 pour le tracking et 2 pour la 3D
        x1=round(x);
        y1=round(y);
        x2=x1;
        y2=y1;
...
save result6 C %save result C pour uniquement le tracking 
